// jobs.js — dsh-bio-gem 后台长任务管理（M1 基建）
// gem_build 是 10min 级任务，不能同步阻塞工具调用：
//   startBuild(args) -> jobId（立即返回）；jobStatus(jobId) -> 进度/结果（TS 轮询）
// 进度事件落 <jobDir>/progress.jsonl（build.py 的 --progress 写入），完成时读取 stdout JSON。
...
// 运行时探测：优先 miniconda（本项目分析环境），回退 PATH python
function pythonExe() {
  const cands = [
...
const jobs = new Map(); // jobId -> {cp, jobDir, progress, done, result, error, started}

function startBuild({ input, name, medium, outDir }) {
  ...
  const rec = { jobId, jobDir, progressFile, stdoutFile, stderrFile,
  ...
  cp.on('exit', (code) => {
    se.end();
    so.end();
    // 从 stdout 文件解析最终 JSON（build.py 最后一行）
    try {
...
function readProgress(jobId) {
  const jobDir = path.join(JOBS_DIR, jobId);
  const pf = path.join(jobDir, 'progress.jsonl');
...
// 轻量 in-memory 任务视图（进程存活期）；磁盘持久视图都用 readProgress/readResult
function jobStatus(jobId) {
  ...
  if (fs.existsSync(resultFile)) {
    ...
    // 进程已退出且无 result.json（异常被杀）
    const stderr = path.join(jobDir, 'stderr.txt');
    error = fs.existsSync(stderr)
      ? fs.readFileSync(stderr, 'utf8').slice(-800) : 'job vanished';
    done = true;
  }
  return { jobId, done, code, result, error, lastEvent: last, events, jobDir,
    elapsed_ms: Date.now() - (m ? m.started : Date.now()) };
}

module.exports = { startBuild, jobStatus, readProgress, pythonExe };